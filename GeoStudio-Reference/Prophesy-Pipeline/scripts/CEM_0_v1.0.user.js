// ==UserScript==
// @name         CEM_0 (v1.0 - FAST)
// @namespace    http://tampermonkey.net/
// @version      1.0.0
// @description  CEM_0 — Triple speed with reduced delays
...
async function clickRadioByValue(value, stepName, fieldLabelHint) {
    console.log('CEM_0: Waiting for radio value: ' + value + (fieldLabelHint ? ' in field: ' + fieldLabelHint : ''));
    ...
    if (!el) {
        console.warn('CEM_0: Radio not found — ' + value);
    ...
    await delay(100);
    el.click();
    console.log('CEM_0: Clicked radio ' + value + (fieldLabelHint ? ' in ' + fieldLabelHint : ''));
    await delay(333);
    return true;
}

async function clickDropdown(fieldId, optionText, stepName) {
    console.log('CEM_0: Waiting for dropdown: ' + fieldId);
    var combo = await waitForElement('#' + fieldId, 15000);

    if (!combo) {
        console.warn('CEM_0: Dropdown not found — ' + fieldId);
        showPopup('Failed: ' + stepName, 3000);
        return false;
    }
    ...
    if (reactKey && combo[reactKey] && combo[reactKey].onMouseDown) {
        ...
        console.log('CEM_0: Opened dropdown ' + fieldId);
    } else {
        combo.click();
        console.log('CEM_0: Clicked dropdown ' + fieldId);
    }
    ...
    if (!list) {
        console.warn('CEM_0: Listbox not found for ' + fieldId);
        showPopup('Failed: ' + stepName, 3000);
        return false;
    }
    ...
    for (var j = 0; j < options.length; j++) {
        if (options[j].textContent.trim() === optionText) {
            ...
            await delay(67);
            options[j].click();
            console.log('CEM_0: Selected "' + optionText + '" in ' + fieldId);
            await delay(333);
            return true;
        }
    }

    console.warn('CEM_0: Option "' + optionText + '" not found in ' + fieldId);
    showPopup('Failed: ' + stepName, 3000);
    return false;
}

async function runFill() {
    showPopup('CEM_0: Filling...', 10000);
    console.log('CEM_0 v1.0: Starting fill');
    ...
    if (!await clickDropdown('cdptype', 'Package Placement -Under Object', 'CDP Type')) return;
    ...
    showPopup('CEM_0: Done!', 2000);
    console.log('CEM_0 v1.0: Fill complete');
}
...
async function injectButton() {
    ...
    if (!ready) { 
        console.warn('CEM_0: Audit form not detected'); 
        return; 
    }

    if (document.getElementById('cem-0-btn')) return;

    var btn = document.createElement('button');
    btn.id = 'cem-0-btn';
    btn.textContent = 'CEM_0';
    var originalBgColor = '#fdd835';
    ...
    console.log('CEM_0 v1.0: Button injected at top: 296px');
}

injectButton();
console.log('CEM_0 v1.0 loaded — iframe');
