...
// @version      1.0.0
// @description  DI_NDPL — Handles cascading/dependent form fields with proper waiting
...
async function clickRadioByValue(value, stepName, fieldLabelHint) {
    console.log('DI_NDPL: Waiting for radio value: ' + value + (fieldLabelHint ? ' in field: ' + fieldLabelHint : ''));
    ...
    if (!el) {
        console.warn('DI_NDPL: Radio not found — ' + value);
    ...
    await delay(300);
    el.click();
    console.log('DI_NDPL: Clicked radio ' + value + (fieldLabelHint ? ' in ' + fieldLabelHint : ''));
    await delay(1000); // Wait for dependent fields to appear
    return true;
}

async function clickDropdown(fieldId, optionText, stepName) {
    console.log('DI_NDPL: Waiting for dropdown: ' + fieldId);
    var combo = await waitForElement('#' + fieldId, 15000);

    if (!combo) {
        console.warn('DI_NDPL: Dropdown not found — ' + fieldId);
        showPopup('Failed: ' + stepName, 3000);
        return false;
    }
    ...
    if (reactKey && combo[reactKey] && combo[reactKey].onMouseDown) {
        ...
        console.log('DI_NDPL: Opened dropdown ' + fieldId);
    } else {
        combo.click();
        console.log('DI_NDPL: Clicked dropdown ' + fieldId);
    }
    ...
    if (!list) {
        console.warn('DI_NDPL: Listbox not found for ' + fieldId);
        showPopup('Failed: ' + stepName, 3000);
        return false;
    }
    ...
    for (var j = 0; j < options.length; j++) {
        if (options[j].textContent.trim() === optionText) {
            ...
            await delay(200);
            options[j].click();
            console.log('DI_NDPL: Selected "' + optionText + '" in ' + fieldId);
            await delay(1000); // Wait for dependent fields to appear
            return true;
        }
    }

    console.warn('DI_NDPL: Option "' + optionText + '" not found in ' + fieldId);
    showPopup('Failed: ' + stepName, 3000);
    return false;
}

async function runFill() {
    showPopup('DI_NDPL: Filling...', 10000);
    console.log('DI_NDPL v1.0: Starting fill');
    ...
    showPopup('DI_NDPL: Done!', 2000);
    console.log('DI_NDPL v1.0: Fill complete');
}
...
async function injectButton() {
    ...
    if (!ready) { 
        console.warn('DI_NDPL: Audit form not detected'); 
        return; 
    }

    if (document.getElementById('di-ndpl-btn')) return;

    var btn = document.createElement('button');
    btn.id = 'di-ndpl-btn';
    btn.textContent = 'DI_NDPL';
    var originalBgColor = '#5c6bc0';
    ...
    btn.addEventListener('click', async function () {
        ...
        try {
            ...
            console.error('DI_NDPL error:', err);
    ...
    console.log('DI_NDPL v1.0: Button injected at top: 12px');
}

injectButton();
console.log('DI_NDPL v1.0 loaded — iframe');
