...
// @version      1.0.0
// @description  FIXED_SDA — Handles cascading/dependent form fields with proper waiting
...
async function clickRadioByValue(value, stepName, fieldLabelHint) {
    console.log('FIXED_SDA: Waiting for radio value: ' + value + (fieldLabelHint ? ' in field: ' + fieldLabelHint : ''));
    ...
    if (!el) {
        console.warn('FIXED_SDA: Radio not found — ' + value);
    ...
    await delay(300);
    el.click();
    console.log('FIXED_SDA: Clicked radio ' + value + (fieldLabelHint ? ' in ' + fieldLabelHint : ''));
    await delay(1000); // Wait for dependent fields to appear
    return true;
}

async function clickDropdown(fieldId, optionText, stepName) {
    console.log('FIXED_SDA: Waiting for dropdown: ' + fieldId);
    var combo = await waitForElement('#' + fieldId, 15000);

    if (!combo) {
        console.warn('FIXED_SDA: Dropdown not found — ' + fieldId);
        showPopup('Failed: ' + stepName, 3000);
        return false;
    }
    ...
    if (reactKey && combo[reactKey] && combo[reactKey].onMouseDown) {
        ...
        console.log('FIXED_SDA: Opened dropdown ' + fieldId);
    } else {
        combo.click();
        console.log('FIXED_SDA: Clicked dropdown ' + fieldId);
    }
    ...
    if (!list) {
        console.warn('FIXED_SDA: Listbox not found for ' + fieldId);
        showPopup('Failed: ' + stepName, 3000);
        return false;
    }
    ...
    for (var j = 0; j < options.length; j++) {
        if (options[j].textContent.trim() === optionText) {
            ...
            await delay(200);
            options[j].click();
            console.log('FIXED_SDA: Selected "' + optionText + '" in ' + fieldId);
            await delay(1000); // Wait for dependent fields to appear
            return true;
        }
    }

    console.warn('FIXED_SDA: Option "' + optionText + '" not found in ' + fieldId);
    showPopup('Failed: ' + stepName, 3000);
    return false;
}

async function runFill() {
    showPopup('FIXED_SDA: Filling...', 10000);
    console.log('FIXED_SDA v1.0: Starting fill');
    ...
    // 4. Audit Resolution — FIXED
    console.log('Step 4: Audit Resolution');
    if (!await clickRadioByValue('FIXED', 'Audit Resolution')) return;
    ...
    // 6. Pod Accuracy — SCANatDifferentAddress
    console.log('Step 6: Pod Accuracy');
    if (!await clickRadioByValue('SCANatDifferentAddress', 'Pod Accuracy', 'pod accuracy')) return;

    // 7. Scan Accuracy — NA
    console.log('Step 7: Scan Accuracy');
    if (!await clickRadioByValue('NA', 'Scan Accuracy', 'scan accuracy')) return;

    // 8. Audit Code — Geocode/Geofence Issue
    console.log('Step 8: Audit Code');
    if (!await clickRadioByValue('Geocode/Geofence Issue', 'Audit Code')) return;
    ...
    showPopup('FIXED_SDA: Done!', 2000);
    console.log('FIXED_SDA v1.0: Fill complete');
}
...
async function injectButton() {
    ...
    if (!ready) { 
        console.warn('FIXED_SDA: Audit form not detected'); 
        return; 
    }

    if (document.getElementById('fixed-sda-btn')) return;

    var btn = document.createElement('button');
    btn.id = 'fixed-sda-btn';
    btn.textContent = 'FIXED_SDA';
    var originalBgColor = '#4a148c';
    ...
    btn.addEventListener('click', async function () {
        ...
        btn.style.backgroundColor = '#33691e';
        try {
            await runFill();
            btn.textContent = 'Done ✓';
        } catch (err) {
            console.error('FIXED_SDA error:', err);
    ...
    console.log('FIXED_SDA v1.0: Button injected at top: 136px');
}

injectButton();
console.log('FIXED_SDA v1.0 loaded — iframe');
