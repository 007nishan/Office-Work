// ==UserScript==
// @name         CEM_1 (v1.0 - FAST)
// @namespace    http://tampermonkey.net/
// @version      1.0.0
// @description  CEM_1 — Triple speed with reduced delays
...
async function clickRadioByValue(value, stepName, fieldLabelHint) {
    console.log('CEM_1: Waiting for radio value: ' + value + (fieldLabelHint ? ' in field: ' + fieldLabelHint : ''));
    ...
    if (!el) {
        console.warn('CEM_1: Radio not found — ' + value);
    ...
    await delay(100);
    el.click();
    console.log('CEM_1: Clicked radio ' + value + (fieldLabelHint ? ' in ' + fieldLabelHint : ''));
    await delay(333);
    return true;
}

async function clickDropdown(fieldId, optionText, stepName) {
    console.log('CEM_1: Waiting for dropdown: ' + fieldId);
    var combo = await waitForElement('#' + fieldId, 15000);

    if (!combo) {
        console.warn('CEM_1: Dropdown not found — ' + fieldId);
        showPopup('Failed: ' + stepName, 3000);
        return false;
    }
    ...
    if (reactKey && combo[reactKey] && combo[reactKey].onMouseDown) {
        ...
        console.log('CEM_1: Opened dropdown ' + fieldId);
    } else {
        combo.click();
        console.log('CEM_1: Clicked dropdown ' + fieldId);
    }
    ...
    if (!list) {
        console.warn('CEM_1: Listbox not found for ' + fieldId);
        showPopup('Failed: ' + stepName, 3000);
        return false;
    }
    ...
    for (var j = 0; j < options.length; j++) {
        if (options[j].textContent.trim() === optionText) {
            ...
            await delay(67);
            options[j].click();
            console.log('CEM_1: Selected "' + optionText + '" in ' + fieldId);
            await delay(333);
            return true;
        }
    }

    console.warn('CEM_1: Option "' + optionText + '" not found in ' + fieldId);
    showPopup('Failed: ' + stepName, 3000);
    return false;
}

async function runFill() {
    showPopup('CEM_1: Filling...', 10000);
    console.log('CEM_1 v1.0: Starting fill');
    ...
    if (!await clickDropdown('cdptype', 'Package Placement -Other Specific Spots', 'CDP Type')) return;
    ...
    showPopup('CEM_1: Done!', 2000);
    console.log('CEM_1 v1.0: Fill complete');
}
...
async function injectButton() {
    ...
    if (!ready) { 
        console.warn('CEM_1: Audit form not detected'); 
        return; 
    }

    if (document.getElementById('cem-1-btn')) return;

    var btn = document.createElement('button');
    btn.id = 'cem-1-btn';
    btn.textContent = 'CEM_1';
    var originalBgColor = '#ffeb3b';
    ...
    console.log('CEM_1 v1.0: Button injected at top: 336px');
}

injectButton();
console.log('CEM_1 v1.0 loaded — iframe');
