...
// @version      1.0.0
// @description  UI_2 — Handles cascading/dependent form fields with proper waiting
...
async function clickRadioByValue(value, stepName, fieldLabelHint) {
    console.log('UI_2: Waiting for radio value: ' + value + (fieldLabelHint ? ' in field: ' + fieldLabelHint : ''));
    ...
    if (!el) {
        console.warn('UI_2: Radio not found — ' + value);
    ...
    await delay(300);
    el.click();
    console.log('UI_2: Clicked radio ' + value + (fieldLabelHint ? ' in ' + fieldLabelHint : ''));
    await delay(1000); // Wait for dependent fields to appear
    return true;
}

async function clickDropdown(fieldId, optionText, stepName) {
    console.log('UI_2: Waiting for dropdown: ' + fieldId);
    var combo = await waitForElement('#' + fieldId, 15000);

    if (!combo) {
        console.warn('UI_2: Dropdown not found — ' + fieldId);
        showPopup('Failed: ' + stepName, 3000);
        return false;
    }
    ...
    if (reactKey && combo[reactKey] && combo[reactKey].onMouseDown) {
        ...
        console.log('UI_2: Opened dropdown ' + fieldId);
    } else {
        combo.click();
        console.log('UI_2: Clicked dropdown ' + fieldId);
    }
    ...
    if (!list) {
        console.warn('UI_2: Listbox not found for ' + fieldId);
        showPopup('Failed: ' + stepName, 3000);
        return false;
    }
    ...
    for (var j = 0; j < options.length; j++) {
        if (options[j].textContent.trim() === optionText) {
            ...
            await delay(200);
            options[j].click();
            console.log('UI_2: Selected "' + optionText + '" in ' + fieldId);
            await delay(1000); // Wait for dependent fields to appear
            return true;
        }
    }

    console.warn('UI_2: Option "' + optionText + '" not found in ' + fieldId);
    showPopup('Failed: ' + stepName, 3000);
    return false;
}

async function runFill() {
    showPopup('UI_2: Filling...', 10000);
    console.log('UI_2 v1.0: Starting fill');
    ...
    if (!await clickRadioByValue('SCANatCDP', 'Pod Accuracy', 'pod accuracy')) return;
    ...
    showPopup('UI_2: Done!', 2000);
    console.log('UI_2 v1.0: Fill complete');
}
...
async function injectButton() {
    ...
    if (!ready) { 
        console.warn('UI_2: Audit form not detected'); 
        return; 
    }

    if (document.getElementById('ui-2-btn')) return;

    var btn = document.createElement('button');
    btn.id = 'ui-2-btn';
    btn.textContent = 'UI_2';
    var originalBgColor = '#6a1b9a';
    ...
    btn.addEventListener('click', async function () {
        ...
        btn.style.backgroundColor = '#9e9d24';
        try {
            await runFill();
            btn.textContent = 'Done ✓';
        } catch (err) {
            console.error('UI_2 error:', err);
    ...
    console.log('UI_2 v1.0: Button injected at top: 256px');
}

injectButton();
console.log('UI_2 v1.0 loaded — iframe');
