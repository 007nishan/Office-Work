...
// @version      1.0.0
// @description  DI_SDA — Handles cascading/dependent form fields with proper waiting
...
async function clickRadioByValue(value, stepName, fieldLabelHint) {
    console.log('DI_SDA: Waiting for radio value: ' + value + (fieldLabelHint ? ' in field: ' + fieldLabelHint : ''));
    ...
    if (!el) {
        console.warn('DI_SDA: Radio not found — ' + value);
    ...
    await delay(300);
    el.click();
    console.log('DI_SDA: Clicked radio ' + value + (fieldLabelHint ? ' in ' + fieldLabelHint : ''));
    await delay(1000); // Wait for dependent fields to appear
    return true;
}

async function clickDropdown(fieldId, optionText, stepName) {
    console.log('DI_SDA: Waiting for dropdown: ' + fieldId);
    var combo = await waitForElement('#' + fieldId, 15000);

    if (!combo) {
        console.warn('DI_SDA: Dropdown not found — ' + fieldId);
        showPopup('Failed: ' + stepName, 3000);
        return false;
    }
    ...
    if (reactKey && combo[reactKey] && combo[reactKey].onMouseDown) {
        ...
        console.log('DI_SDA: Opened dropdown ' + fieldId);
    } else {
        combo.click();
        console.log('DI_SDA: Clicked dropdown ' + fieldId);
    }
    ...
    if (!list) {
        console.warn('DI_SDA: Listbox not found for ' + fieldId);
        showPopup('Failed: ' + stepName, 3000);
        return false;
    }
    ...
    for (var j = 0; j < options.length; j++) {
        if (options[j].textContent.trim() === optionText) {
            ...
            await delay(200);
            options[j].click();
            console.log('DI_SDA: Selected "' + optionText + '" in ' + fieldId);
            await delay(1000); // Wait for dependent fields to appear
            return true;
        }
    }

    console.warn('DI_SDA: Option "' + optionText + '" not found in ' + fieldId);
    showPopup('Failed: ' + stepName, 3000);
    return false;
}

async function runFill() {
    showPopup('DI_SDA: Filling...', 10000);
    console.log('DI_SDA v1.0: Starting fill');
    ...
    showPopup('DI_SDA: Done!', 2000);
    console.log('DI_SDA v1.0: Fill complete');
}
...
async function injectButton() {
    ...
    if (!ready) { 
        console.warn('DI_SDA: Audit form not detected'); 
        return; 
    }

    if (document.getElementById('di-sda-btn')) return;

    var btn = document.createElement('button');
    btn.id = 'di-sda-btn';
    btn.textContent = 'DI_SDA';
    var originalBgColor = '#0d47a1';
    ...
    btn.addEventListener('click', async function () {
        ...
        try {
            ...
            console.error('DI_SDA error:', err);
    ...
    console.log('DI_SDA v1.0: Button injected at top: 96px');
}

injectButton();
console.log('DI_SDA v1.0 loaded — iframe');
